...
| Structured Streaming resumes with offset gap or `OFFSET_OUT_OF_RANGE` error | Stream was paused longer than the Event Hub retention window; messages in the gap are no longer available | Detect the gap: compare `MAX(enqueuedTime)` in the Delta table against the Event Hub's earliest available offset in the Azure portal. If a gap exists and Event Hubs Capture is enabled, backfill from the capture path using COPY INTO or Auto Loader. If no secondary source exists, document the data loss and increase the Event Hub retention period to reduce future risk. |
