...
> **Data is stored in ADLS Gen2, not Databricks.** Databricks is a compute and orchestration platform; all table data, checkpoints, and Delta files are stored in your own Azure Data Lake Storage Gen2 account. This is why every ingestion method in this guide requires an ADLS Gen2 storage account and a Unity Catalog external location to be configured before data can be written. See [Azure Databricks high-level architecture](https://learn.microsoft.com/en-us/azure/databricks/getting-started/high-level-architecture) for details.

**Quick navigation:**

| If your source is... | Use this method | Jump to implementation |
|----------------------|-----------------|------------------------|
| Files arriving in cloud storage (continuous) | Auto Loader | [File Ingestion: Auto Loader](#file-ingestion-auto-loader) |
| Files arriving on a schedule (batch) | COPY INTO | [File Ingestion: COPY INTO](#file-ingestion-copy-into) |
| Kafka / Azure Event Hubs — sub-minute latency required | Structured Streaming | [Streaming Ingestion: Structured Streaming](#streaming-ingestion-structured-streaming) |
| Kafka / Azure Event Hubs — batch/micro-batch acceptable | Kafka Connector → ADLS Gen2, then Auto Loader or COPY INTO | [File Ingestion: Auto Loader](#file-ingestion-auto-loader) / [COPY INTO](#file-ingestion-copy-into) |
| Relational database (SQL Server, PostgreSQL) | JDBC | [Database Ingestion: JDBC](#database-ingestion-jdbc) |
| SaaS application (Salesforce, Workday) | Lakeflow Connect | [Managed Ingestion: Lakeflow Connect](#managed-ingestion-lakeflow-connect) |
| Any other source (ERP, mainframe, on-premises, SFTP partner delivery, custom API) | Land files in ADLS Gen2, then Auto Loader or COPY INTO | [Sources Not Covered](#sources-not-covered-in-this-cookbook) |

If your source type matches more than one option, or if latency, schema evolution, or cost constraints affect the choice, work through the **[Method Selection](#method-selection)** section before jumping to an implementation section.
...
| **Structured Streaming** | Sub-minute latency ingestion from Kafka or Azure Event Hubs; event-driven architectures where consumer lag must be minimised; stateful aggregations with watermarking. When sub-minute latency is not required, a Kafka Connector (e.g. Confluent Kafka Connect ADLS Sink) can land messages as files in ADLS Gen2 and Auto Loader or COPY INTO can ingest them on a batch schedule — this avoids running a continuous Spark stream. | The source is cloud storage files rather than a message bus; your team lacks the operational capability to manage streaming job recovery |
...
Choosing the wrong schema evolution mode causes silent data loss (columns dropped without error) or unexpected pipeline failures. Review the behaviour for your chosen method before implementing.
...
| **Lakeflow Connect** | Yes — automatic | New source columns automatically added on the next pipeline run. Prior rows have `null` for the new column. Deleted source columns retained in Delta with `null`. Column renames produce a new column; the prior column persists with historical values. |

### Batch vs. Streaming

If you are choosing between a scheduled batch load and a streaming approach, compare on these dimensions:

| Dimension | Batch | Micro-Batch (`trigger(availableNow=True)`) | Continuous Streaming |
|-----------|-------|---------------------------------------------|----------------------|
...
| **Failure recovery** | Re-run from the start of the batch window | Resume from checkpoint | Resume from last committed offset; Kafka/Event Hubs retention must be sufficient |
