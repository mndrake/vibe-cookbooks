...
In native Databricks pipelines, incremental loading (processing only new or changed records since the last run) is implemented using Databricks Jobs with explicit watermark logic in each task. This section shows the three main native equivalents to dbt's incremental materialisation strategies.
...
Dynamic pivot using PySpark (equivalent to `dbt_utils.pivot()`):
...
SQL pivot (static column list, equivalent to `dbt_utils.pivot()` with known values):
...
`dbt_utils.date_spine()` generates a dense sequence of dates to ensure no dates are missing from a time series. In native Spark, the same result is achieved with `sequence()` + `explode()` (SQL or PySpark) or with `date_add()` combined with a range.
...
- **`sequence()` is native to Spark SQL and PySpark:** No external packages are required, unlike `dbt_utils.date_spine()` which requires the dbt-utils package.
...
- **Equivalence to dbt run / dbt test:** The Databricks Jobs DAG replaces `dbt run` (notebook tasks execute the transformations) and `dbt test` (the quality assertions task runs post-pipeline checks). The bundle YAML replaces `dbt_project.yml` as the pipeline definition file checked into version control.
...
- **CI/CD integration:** Add `databricks bundle validate` and `databricks bundle deploy --target staging` as CI steps in your Git provider's pipeline (GitHub Actions, Azure DevOps, GitLab CI). This deploys the bundle to a staging environment on every pull request merge, equivalent to a dbt CI run.
