# Security Architectural Patterns (Native Databricks)

> **Scope note:** This file covers security architectural patterns using **native Databricks and Unity Catalog features only** — no dbt. If you are running a dbt-based pipeline and want to layer dbt grants, dbt meta tags, and dbt-specific Unity Catalog integration on top of these patterns, see the dbt version at [`../../../databricks_and_dbt/security/security_patterns.md`](../../../databricks_and_dbt/security/security_patterns.md).
...
- **Pipeline Service Principal Privilege Model** — the minimum privilege set for a service principal scoped to Data Vault layers
...
|--------|-----------------|-----------|-----------|
| `raw_vault` | Pipeline service principal | `USE SCHEMA`, `CREATE TABLE`, `SELECT` | Pipeline materialisation and cross-satellite joins |
...
| `business_vault` | Pipeline service principal | `USE SCHEMA`, `CREATE TABLE`, `SELECT` on `raw_vault` | Materialises PITs, bridges, derived satellites |
...
| `marts` | Pipeline service principal | `USE SCHEMA`, `CREATE TABLE`, `SELECT` on `business_vault` | Materialises information mart tables |
...
The following table defines the minimum privilege set for each layer in a native Databricks Data Vault pipeline:

| Schema | Required Privilege | Purpose |
|--------|-------------------|---------|
| Catalog (`main`) | `USE CATALOG` | Navigate the catalog |
| `staging` | `USE SCHEMA`, `SELECT` (on raw source tables) | Read raw ingested data to build staging transformations |
| `raw_vault` | `USE SCHEMA`, `CREATE TABLE`, `SELECT` | Materialise hub, link, and satellite tables; read them for downstream steps |
...
In practice, if a single service principal runs all pipeline steps across all layers, it needs `SELECT` on `raw_vault`, `business_vault`, and `marts` schemas simultaneously (because cross-schema queries in notebooks resolve at query time). The SP's `SELECT` access to raw schemas should therefore be treated as an operational requirement, not a privilege escalation — the SP is not a data consumer, and its access to raw satellites does not expose PII to human users.
...
| Layer | Access Control Approach |
|-------|------------------------|
| **Staging (transient views/CTEs)** | No persistent security objects needed — staging is transient. The underlying raw table access is controlled at the raw schema level. |
