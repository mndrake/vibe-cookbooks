# Security Cookbook (Native Databricks)

> **Scope note:** This file covers security implementation using **native Databricks and Unity Catalog features only** — no dbt. If you are running a dbt-based pipeline and want to layer dbt grants, dbt meta tags, and dbt-specific Unity Catalog integration on top of these patterns, see the dbt version at [`../../../databricks_and_dbt/security/security_cookbook.md`](../../../databricks_and_dbt/security/security_cookbook.md).

---

## Introduction

This cookbook provides practical, step-by-step guidance for **implementing data security and governance** on Databricks with Unity Catalog using only native platform features. It covers role-based access control, secrets management, row-level security, column masking, Unity Catalog tag management, audit logging, encryption configuration, and service principal management. It is intended to be self-contained — no prior knowledge of Unity Catalog governance is assumed.
...
Use Unity Catalog `GRANT` statements scoped to Databricks groups. The following example sets up a complete privilege structure for a Data Vault pipeline with three roles: a pipeline service principal (`pipeline_service_principal`), a data engineering group (`data_engineers`), and an analyst group (`analysts`).
...
Unlike dbt (which uses a `grants` config block to automate post-materialisation grants), native Databricks pipelines require explicit grant re-application when tables are recreated. Use one of the following patterns:
...
- **Tags are queryable via system catalog:** Unlike dbt `schema.yml` meta properties (which live in project YAML files and require `dbt docs generate` to become visible), Unity Catalog tags are live metadata queryable by any user with access to `system.information_schema`. This makes them suitable as the authoritative, real-time source for compliance reporting.
