...
- **Data Vault architectures favour landing zone + Auto Loader:** The Raw Vault loading principle (never modify source data, load exactly as received) is most naturally satisfied by landing raw files and processing them via Auto Loader into a DLT staging layer before hashing. JDBC or connector-based ingestion can also feed a Raw Vault, but the raw file is not retained.
