...
> **Scope note:** All patterns are implemented with PySpark, Spark SQL, Delta Lake, Delta Live Tables (DLT), and Databricks Jobs.
...
- **Unity Catalog SQL functions are persistent:** Unity Catalog SQL functions are stored in the catalog and callable from any notebook, DLT pipeline, or SQL Warehouse in the same catalog without re-registration.
...
Databricks Jobs is the native orchestration layer for batch and streaming pipelines. It provides a DAG of tasks that can include notebook tasks, Python script tasks, SQL tasks, DLT pipeline tasks, and Databricks Asset Bundle deployments.
