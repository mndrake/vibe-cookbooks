---
name: review-cookbook
description: Check a cookbook for structural compliance against project conventions. Use when the user wants to validate that a cookbook follows the required section order, naming rules, and content requirements.
...
| # | Rule | Pass condition |
|---|------|----------------|
| 1 | **Top-level H2 sections are domain types** | Every H2 (except Introduction, Design Decisions, Dev Pre-Requisites, Infrastructure Pre-Requisites, Managing Your Environment) names a domain type — not a method name |
...
Produce a summary table of all checks with pass/fail status, then list each failure with its
line number and corrective action.

If all checks pass, state that the file is compliant and ready for `/critique-cookbook`.
